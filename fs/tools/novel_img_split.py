    ...
    for i in range(windows.shape[0]):
        ...
        if bboxes_num == 0:
            ...
            for idx in range(bboxes_num):
                ...
                diff = obj['diffs'][idx]
                if diff < 10:
                    diffs = str(diff) if not obj['trunc'][idx] else '2'
                    if diffs != '2':
                        all_ignore = False
                else:
                    diffs = str(diff)
...
def main(args=None):
    ...
    ### 将 seed shot 放入 args
    mask_hint = "mask_" if args.mask else ""
    seed_shot_dir = f"{mask_hint}seed{args.seed}_shot{args.shot}"
    args.img_dirs = [osp.join(img_dir, seed_shot_dir, "images") for img_dir in (args.img_dirs)]
    args.ann_dirs = [osp.join(ann_dir, seed_shot_dir, "labelTxt") for ann_dir in (args.ann_dirs)]
    args.save_dir = osp.join(args.save_dir, seed_shot_dir)
    print(f"Save split results into {args.save_dir}")
...
if __name__ == '__main__':
    ...
    args = sys.argv[1:]
    if len(args) == 0:
        args = ["--base-json=fs/tools/split_configs/ss_train_split.json", "--seed=5", "--shot=30"]
    main(args)
