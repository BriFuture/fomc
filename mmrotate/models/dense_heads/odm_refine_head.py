...
from gdet.registries import MODEL_HEADS as ROTATED_HEADS
from ..utils.orconv import ORConv2d
from ..utils.ripool import RotationInvariantPooling
...
class ODMRefineHead(RotatedRetinaHead):
    ...
    def __init__(self,
        ...
        if anchor_generator is None:
            anchor_generator = dict(type='PseudoAnchorGenerator', strides=[8, 16, 32, 64, 128])
        if init_cfg is None:
            init_cfg = dict(type='Normal', layer='Conv2d', std=0.01,
                override=dict(type='Normal', name='odm_cls', std=0.01, bias_prob=0.01)
            )
