...
import numpy as np
import torch
from gdet.losses.base_losses import softmax_cross_entropy
from ..datasets.evaluator import coco_metrics
...
class Accuracy(metrics.Average):
    ...
    @classmethod
    def from_model_output(cls, logits: "torch.Tensor", labels: "torch.Tensor", **kwargs):
        if logits.shape != labels.shape:
            raise ValueError(
                "labels and logits shapes must be compatible"
                f"{labels.shape} != {logits.shape}"
            )
        matches: "torch.Tensor" = logits.argmax(axis=-1) == labels.argmax(axis=-1)
...
class AccuracyFromDictionary(metrics.Average):
    ...
    @classmethod
    def from_model_output(cls, model_outputs: "dict", labels: "dict", **kwargs):
        ...
        if "targets" in model_outputs:
            targets: "np.ndarray" = model_outputs["targets"]
        elif "labels" in model_outputs:
            targets: "np.ndarray" = model_outputs["labels"]
        elif isinstance(labels, dict) and "mc_labels" in labels:
            targets: "np.ndarray" = labels["mc_labels"]
        ...
        if predictions.shape != targets.shape:
            raise ValueError(
                "targets and predictions shapes must be compatible"
                f"{targets.shape} != {predictions.shape}"
            )
        matches: "torch.Tensor" = predictions == targets
        matches = matches.to(torch.float32)
        return super().from_model_output(values=matches, **kwargs)
...
def write_metrics(
    step, eval_metrics, summary_writer, direct_write_keys=False, strip_task_scope=False
):
    """Helper function to write the results of evaluation metrics to tensorboard.
