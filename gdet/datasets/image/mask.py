...
class BitmapMasks(BaseInstanceMasks):
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            new_w, new_h = geometric.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.uint8)
        else:
            rescaled_masks = np.stack([
                geometric.imrescale(mask, scale, interpolation=interpolation)
    ...
    def resize(self, out_shape, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            ...
            resized_masks = np.stack([
                geometric.imresize(mask, out_shape, interpolation=interpolation)
                for mask in self.masks
            ])
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([
                geometric.imflip(mask, direction=flip_direction)
                for mask in self.masks
            ])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([
                geometric.impad(mask, shape=out_shape, pad_val=pad_val)
                for mask in self.masks
            ])
    ...
    def translate(self,
        ...
        if len(self.masks) == 0:
            ...
            translated_masks = geometric.imtranslate(
    ...
    def shear(self,
        ...
        if len(self.masks) == 0:
            ...
            sheared_masks = geometric.imshear(
                self.masks.transpose((1, 2, 0)),
                magnitude,
                direction,
    ...
    def rotate(self, out_shape, angle, center=None, scale=1.0, fill_val=0):
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = geometric.imrotate(
                self.masks.transpose((1, 2, 0)),
                angle,
                center=center,
                scale=scale,
    ...
    def areas(self):
        ...
        for polygons_per_obj in self.masks:
            ...
            for p in polygons_per_obj:
                area_per_obj = area_per_obj + self._polygon_area(p[0::2], p[1::2])
...
def polygon_to_bitmap(polygons: "list[np.ndarray]", height: "int", width: "int"):
