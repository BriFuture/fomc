...
def convert_predictions_to_coco_annotations(
    predictions: "ModelPredictions", remove_invalid_boxes=False
):
    ...
    for i in range(num_batches):
        ...
        detect_boxes = predictions["detection_boxes"]
        for bi in range(batch_size):
            detect_boxes[i][bi] = box_utils.xyxy_to_xywh(detect_boxes[i][bi])
        if use_outer_box:
            predictions["detection_outer_boxes"][i] = box_utils.xyxy_to_xywh(
                predictions["detection_outer_boxes"][i]
            )
            mask_boxes = predictions["detection_outer_boxes"]
        else:
            mask_boxes = detect_boxes
